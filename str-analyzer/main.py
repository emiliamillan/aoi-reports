...
def main() -> None:
    ...
    path = input("Path of Raw Data:")

    try:
        # 1. LOAD EXCEL
        sheet_1, sheet_2 = load_excel(path)
        print("\nExcel file loaded successfully!")
        print(f"Sheet 1 rows: {len(sheet_1)}")
        print(f"Sheet 2 rows: {len(sheet_2)}")

        # 2. GET STR VALUES
        value = input("\nEnter STRs separated by commas: ")
        values = value.split(",")
        ...
        # 9. Output File
        output_path = ("str-analyzer/filtered_results.xlsx")

        with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
            # Filtered Results sheet
            results_with_total.to_excel(writer, sheet_name="Filtered Results", index=False)
